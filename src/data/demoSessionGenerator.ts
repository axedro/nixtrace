import { DEMO_FLOW_5GSA, DEMO_FLOW_VONR } from './demoFlows';
...
export function generateDemoSession(): Session {
  ...
  const clone = structuredClone(base) as Session;

  // Always refresh identity + timestamp
  ...
  // Shift all message timestamps to "now"
  ...
  switch (scenario) {
    case 'A1': {
      ...
      // Slight jitter on timing KPIs for realism
    ...
    case 'A2': {
      ...
      clone.kpis = {
        setupTimeMs: 42,
        cause5gmm:   '21 — MAC failure (AUTN mismatch)',
        authRttMs:   29,
      };
      return clone;
    }
